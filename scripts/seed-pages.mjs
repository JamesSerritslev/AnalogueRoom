...
async function main() {
  ...
  await withRetries("Save pageMenus", async () => {
    const existingMenus = await client.fetch(`*[_id == $id][0]{_id, heroBackground}`, {
      id: IDS.pageMenus,
    })
    const menuFields = {
    ...
    if (existingMenus?._id) {
      return client.patch(IDS.pageMenus).set(menuFields).commit()
    }
    return client.createOrReplace({
      _id: IDS.pageMenus,
      _type: "pageMenus",
      ...menuFields,
      heroBackground: menuBgImg ?? interiorImg,
    })
  })
